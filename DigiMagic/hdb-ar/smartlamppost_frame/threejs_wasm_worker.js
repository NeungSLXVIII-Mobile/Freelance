...
function start2(container, marker, video, input_width, input_height, canvas_draw, render_update, track_update) {
    ...
    var draw = function () {
        ...
        if (p1_show) {
            // P1.
            if (p1_bicycle_textures[0] != undefined) {
                p1_bicycle_textures[0].dispose();
            }
            p1_bicycle_textures[0] = new THREE.TextureLoader().load(p1_bicycle_path + zero_path(p1_bicycle_frame) + p1_bicycle_frame + '.png');
            p1_bicycle_material.map = p1_bicycle_textures[0];
            p1_bicycle_frame++;
            if (p1_bicycle_frame >= p1_bicycle_frame_count) {
                p1_bicycle_frame = 0;
            }

            if (p1_human2_textures[0] != undefined) {
                p1_human2_textures[0].dispose();
            }
            p1_human2_textures[0] = new THREE.TextureLoader().load(p1_human2_path + zero_path(p1_human2_frame) + p1_human2_frame + '.png');
            p1_human2_material.map = p1_human2_textures[0];
            p1_human2_frame++;
            if (p1_human2_frame >= p1_human2_frame_count) {
                p1_human2_frame = 0;
            }

            if (p1_human3_textures[0] != undefined) {
                p1_human2_textures[0].dispose();
            }
            p1_human3_textures[0] = new THREE.TextureLoader().load(p1_human3_path + zero_path(p1_human3_frame) + p1_human3_frame + '.png');
            p1_human3_material.map = p1_human3_textures[0];
            p1_human3_frame++;
            if (p1_human3_frame >= p1_human3_frame_count) {
                p1_human3_frame = 0;
            }

            if (p1_human4_textures[0] != undefined) {
                p1_human4_textures[0].dispose();
            }
            p1_human4_textures[0] = new THREE.TextureLoader().load(p1_human4_path + zero_path(p1_human4_frame) + p1_human4_frame + '.png');
            p1_human4_material.map = p1_human4_textures[0];
            p1_human4_frame++;
            if (p1_human4_frame >= p1_human4_frame_count) {
                p1_human4_frame = 0;
            }

            if (p1_human5_textures[0] != undefined) {
                p1_human5_textures[0].dispose();
            }
            p1_human5_textures[0] = new THREE.TextureLoader().load(p1_human5_path + zero_path(p1_human5_frame) + p1_human5_frame + '.png');
            p1_human5_material.map = p1_human5_textures[0];
            p1_human5_frame++;
            if (p1_human5_frame >= p1_human5_frame_count) {
                p1_human5_frame = 0;
            }

            if (p1_human6_textures[0] != undefined) {
                p1_human6_textures[0].dispose();
            }
            p1_human6_textures[0] = new THREE.TextureLoader().load(p1_human6_path + zero_path(p1_human6_frame) + p1_human6_frame + '.png');
            p1_human6_material.map = p1_human6_textures[0];
            p1_human6_frame++;
            if (p1_human6_frame >= p1_human6_frame_count) {
                p1_human6_frame = 0;
            }

            if (p1_icon_textures[0] != undefined) {
                p1_icon_textures[0].dispose();
            }
            p1_icon_textures[0] = new THREE.TextureLoader().load(p1_icon_path + '_' + p1_icon_frame + '.png');
            p1_icon_material.map = p1_icon_textures[0];
            p1_icon_frame++;
        ...
        if (p2_show) {
            // P2.
            if (p2_human1_textures[0] != undefined) {
                p2_human1_textures[0].dispose();
            }
            p2_human1_textures[0] = new THREE.TextureLoader().load(p2_human1_path + zero_path(p2_human1_frame) + p2_human1_frame + '.png');
            p2_human1_material.map = p2_human1_textures[0];
            p2_human1_frame++;
            if (p2_human1_frame >= p2_human1_frame_count) {
                p2_human1_frame = 0;
            }

            if (p2_human2_textures[0] != undefined) {
                p2_human2_textures[0].dispose();
            }
            p2_human2_textures[0] = new THREE.TextureLoader().load(p2_human2_path + zero_path(p2_human2_frame) + p2_human2_frame + '.png');
            p2_human2_material.map = p2_human2_textures[0];
            p2_human2_frame++;
            if (p2_human2_frame >= p2_human2_frame_count) {
                p2_human2_frame = 0;
            }

            if (p2_human3_textures[0] != undefined) {
                p2_human3_textures[0].dispose();
            }
            p2_human3_textures[0] = new THREE.TextureLoader().load(p2_human3_path + zero_path(p2_human3_frame) + p2_human3_frame + '.png');
            p2_human3_material.map = p2_human3_textures[0];
            p2_human3_frame++;
            if (p2_human3_frame >= p2_human3_frame_count) {
                p2_human3_frame = 0;
            }

            if (p2_human4_textures[0] != undefined) {
                p2_human4_textures[0].dispose();
            }
            p2_human4_textures[0] = new THREE.TextureLoader().load(p2_human4_path + zero_path(p2_human4_frame) + p2_human4_frame + '.png');
            p2_human4_material.map = p2_human4_textures[0];
            p2_human4_frame++;
            if (p2_human4_frame >= p2_human4_frame_count) {
                p2_human4_frame = 0;
            }

            if (p2_human5_textures[0] != undefined) {
                p2_human5_textures[0].dispose();
            }
            p2_human5_textures[0] = new THREE.TextureLoader().load(p2_human5_path + zero_path(p2_human5_frame) + p2_human5_frame + '.png');
            p2_human5_material.map = p2_human5_textures[0];
            p2_human5_frame++;
            if (p2_human5_frame >= p2_human5_frame_count) {
                p2_human5_frame = 0;
            }

            if (p2_icon_textures[0] != undefined) {
                p2_icon_textures[0].dispose();
            }
            p2_icon_textures[0] = new THREE.TextureLoader().load(p2_icon_path + '_' + p2_icon_frame + '.png');
            p2_icon_material.map = p2_icon_textures[0];
            p2_icon_frame++;
        ...
        if (p3_show) {
            ...
            if (p3_icon_textures[0] != undefined) {
                p3_icon_textures[0].dispose();
            }
            p3_icon_textures[0] = new THREE.TextureLoader().load(p3_icon_path + '_' + p3_icon_frame + '.png');
            p3_icon_material.map = p3_icon_textures[0];
            p3_icon_frame++;
